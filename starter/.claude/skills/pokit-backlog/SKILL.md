<!-- Seed maintained in the pokit2 package (starter/). Edit directly. -->
...
```bash
pokit issue:create --title "<issue title>"
pokit issues
pokit doctor
```

Use `pokit issue:create` for issue creation receipts and `pokit issues` to confirm the available IDs. Active issue selection is managed by the agent workflow. Do not imply a mutation receipt exists unless a CLI command or an explicit manual trace actually created it.
...
For PO-facing output labels, use `.ai-os/standards/terminology.md` Quick Map. Keep internal event names in English, but prefer "기록" over "영수증" in user-facing text.
